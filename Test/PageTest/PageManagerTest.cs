...
public class PageManagerTest
{
    ...
    {
        var page = await _manager.AllocatePage<DataPage>(false);
        int id = page.Header.Id;
        
        Assert.Equal(0, id);
        
        var read = await _manager.LoadPage<DataPage>(id, false);
    ...
    {
        DataPage page = await _manager.AllocatePage<DataPage>(false);
        ...
        var read = await _manager.LoadPage<DataPage>(page.Header.Id, false);
    ...
    {
        var dataPage = await _manager.AllocatePage<DataPage>(false);
        var indexPage = await _manager.AllocatePage<IndexPage>(false);
        var leafPage = await _manager.AllocatePage<LeafPage>(false);
    ...
    {
        var p1 = await _manager.AllocatePage<DataPage>(false);
        var p2 = await _manager.AllocatePage<DataPage>(false);
